...
C=zeros(1,n);
for i=1:2*N  %generate N decays with an exponential distribution
    ibin=ceil(-(tau/(dt/2))*log(rand(1)));
    if(ibin<=n)
        C(ibin)=C(ibin)+1;
    end
end
...
fprintf('the difference in tau values is %g \n\n', abs(newtau-oldtau));
...
fprintf('lowertau, %g, confidence, %g, \n',lowertau,confidence(lowertau,v,chi2,n,percent)+percent);
...
fprintf('uppertau, %g, confidence, %g, \n',uppertau,confidence(uppertau,v,chi2,n,percent)+percent);
